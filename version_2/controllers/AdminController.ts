...
import { sendPaymentConfirmation, sendPayoutAlert } from '../services/NotificationService';
...
export const processCyclePayout = async (req: Request, res: Response) => {
  try {
    ...
    // Logic for amount: payout depends with the number of members
    const memberCount = await User.countDocuments({ role: 'member' });
    const payoutAmount = memberCount * 500; // 500 KES contribution per member
    ...
    // Rotate the queue: Move the person who just got paid to the end
    ...
    config.cycleOrder = userIds;
    // We can keep currentIndex as 0 if we rotate, because the next person shifted up.
    // Or we can just increment currentIndex if we don't rotate.
    // Rotation is usually preferred for "Cycle" logic so the UI always shows who's next at the top.
    config.currentIndex = 0; 
    config.cycleDay = 1; // Reset day to 1 for the next payout recipient
    await config.save();

    // Update all users payout positions to reflect the new order
