...
export const processB2BPayout = async (req: Request, res: Response) => {
    try {
        ...
        // Logic for payout amount - e.g. sum of current pool or fixed reward
        const payoutAmount = 5000; // Placeholder fixed payout
...
export const handleB2BResult = async (req: Request, res: Response) => {
    ...
    if (result.ResultCode === 0) {
        // Success
        // Update SystemConfig index for next rotation
        const config = await SystemConfig.findOne();
        if (config) {
