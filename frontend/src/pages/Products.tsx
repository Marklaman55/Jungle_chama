  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] atmosphere-gradient">
              ...
              <div className="md:w-2/5 p-8 flex flex-col">
                <div className="flex-1 overflow-y-auto">
                  <div className="flex items-center gap-3 mb-6">
                    <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${selectedProduct.status === 'approved' ? 'bg-green-100 text-green-600' : 'bg-blue-100 text-blue-600'}`}>{selectedProduct.status}</span>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-8">
                    <div className="bg-gray-50 p-4 rounded-2xl"><p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Available Stock</p><p className="text-lg font-black text-black">{selectedProduct.stock} Units</p></div>
                  </div>
                  {selectedProduct.trackerId && <div className="p-4 bg-black/5 rounded-2xl border border-black/5 flex items-center justify-between mb-4"><div><p className="text-[8px] font-black text-gray-400 uppercase tracking-[0.2em] mb-0.5">Product Tracker ID</p><p className="text-xs font-mono font-bold text-black">{selectedProduct.trackerId}</p></div><div className="w-8 h-8 rounded-full bg-jungle/10 flex items-center justify-center"><CheckCircle2 size={14} className="text-jungle" /></div></div>}
                </div>
                <div className="mt-10 space-y-3">
                  <button onClick={() => { setShowProductDetails(false); }} className="w-full py-6 bg-black text-white rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-jungle transition-all"><ArrowRight className="rotate-180 mr-2" size={18} /> Back to Collection</button>
                </div>
              </div>
            </motion.div>
          </motion.div>
      ...
      <footer className="mt-20 py-10 border-t border-gray-100 text-center">
        <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em]">© 2026 THE JUNGLE SAVINGS CHAMA. ALL RIGHTS RESERVED.</p>
      </footer>
    </div>
  );
};
