    ...
    try {
      ...
      for (const user of users) {
        ...
        if (user.carryForward < 0) {
          // Mark user as owing - for now just log and send message
      ...
      if (config.cycleDay > 10) {
        ...
        if (payoutUser) {
          const payoutAmount = 1000; // Example payout amount
        ...
        // If all users have been paid, maybe change state?
        if (config.currentIndex >= config.cycleOrder.length) {
          config.systemState = 'BREAK';
          config.currentIndex = 0; // Reset for next cycle
        }
      }
